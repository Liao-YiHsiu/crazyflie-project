...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret == True:#判断视频是否结束
            ...
            erosion = cv2.erode(thresheld,kernel,iterations = 1)
            ...
            if hsv_set is False and len(contours) >0 :
                for i in contours:
                    cnt = i-1
                    M = cv2.moments(cnt)
                    if M['m00']>0 :#cv2.contourArea(cnt)>max_area:
                        if cv2.contourArea(cnt)>max_area:
                            max_area = cv2.contourArea(cnt)
                        cx = int(M['m10']/M['m00'])
                        cy = int(M['m01']/M['m00'])
                        if (pre_cx<0 and pre_cy<0)or(math.fabs(cx-pre_cx)<20 and math.fabs(cx-pre_cy),20):
                            pre_cx=cx
                            pre_cy=cy
